...
class DicePlugin(Star):
    ...
    async def handle_roll_dice(self, event: AstrMessageEvent, message: str = None):
        """普通掷骰，支持添加判定值和快捷判定"""
        full_command = message.strip() if message else f"1d{DEFAULT_DICE}"
        ...
        if len(parts) == 1:
            try:
                ...
                # 如果失败（例如指令是 /r 3d6），则它就是一个普通的投骰表达式
                dice_expression = parts[0]
        # 情况2: 指令是 `/r 1d100 75` 这样的标准格式
        elif len(parts) > 1:
            dice_expression = parts[0]
            try:
        ...
        if not dice_expression: # 兜底处理 /r 的情况
            dice_expression = f"1d{DEFAULT_DICE}"
